...
@Component({
  templateUrl: 'addoem.component.html',
  styleUrls: ['../../../scss/vendors/toastr/toastr.scss']
})

@Injectable()
export class AddoemComponent {
...
/* 			
	  ...
	  /*   let roles = NgxRolesService.getRoles();
	    ...
	    if(this.editparam.id != undefined) {
	    	...
	    	this.http.get(API_URL+'/Oems/'+ this.editparam.id +'?access_token='+ localStorage.getItem('currentUserToken'), options)
	        .subscribe(response => {
	...
	onSubmit() {
		...
		if(this.editparam.id  == undefined){
			...
			this.http.post(API_URL+'/Oems?access_token='+localStorage.getItem('currentUserToken'), this.data,  options)
			.subscribe(response => {
				console.log(response.json());		
			...
			this.http.post(API_URL+'/Oems/update?where=%7B%22id%22%3A%22'+  this.editparam.id +'%22%7D&access_token='+ localStorage.getItem('currentUserToken'), this.data,  options)
	        .subscribe(response => {
	        	console.log(response.json());			
			    this.router.navigate(['oem']);  
		    }, error => {
