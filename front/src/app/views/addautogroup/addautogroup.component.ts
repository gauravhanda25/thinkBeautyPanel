...
@Component({
  templateUrl: 'addautogroup.component.html',
  styleUrls: ['../../../scss/vendors/toastr/toastr.scss'],
})

@Injectable()
export class AddautogroupComponent {
...
/* 			
	  ...
	  /*   let roles = NgxRolesService.getRoles();
	    ...
	    if(this.editparam.id != undefined) {
	    	...
	    	this.http.get(API_URL+'/AutoGroups/'+ this.editparam.id +'?access_token='+ localStorage.getItem('currentUserToken'), options)
	        .subscribe(response => {
	        	console.log(response.json());	
			 ...
			 this.http.get(API_URL+'/AutoGroupContacts/?filter=%7B%22where%22%3A%7B%22or%22%3A%5B%7B%22autoGroupId%22%3A%22'+this.editparam.id+'%22%7D%5D%7D%7D&access_token='+ localStorage.getItem('currentUserToken'), options)
	        .subscribe(response => {
	        	console.log(response.json());	
	        	...
	        	if(response.json().length != 0) {
		        	...
		        	for(let res in response.json()){
		        		...
		        		this.contactdata.autoGroupId = response.json()[res].autoGroupId;
	...
	saveContacts(){
		...
		for(let key in this.contactdata.contactperson){
			...
			this.http.post(API_URL+'/AutoGroupContacts?access_token='+localStorage.getItem('currentUserToken'), this.val,  options)
	        .subscribe(response => {
	...
	updateContacts(){
		...
		this.http.delete(API_URL+'/AutoGroups/'+this.editparam.id+'/autogroupcontacts?access_token='+localStorage.getItem('currentUserToken'), options)
		.subscribe(response => {
			...
			for(let key in this.contactdata.contactperson){
				...
				.subscribe(response => {
					console.log(response.json().id);
					this.toasterService.pop('success', 'Success ', "Auto Groups Contacts updated successfully.");	
	...
	onSubmit() {
		...
		if(this.editparam.id  == undefined){
			...
			this.http.post(API_URL+'/AutoGroups?access_token='+localStorage.getItem('currentUserToken'), this.data,  options)
			.subscribe(response => {
				console.log(response.json());
				this.router.navigate(['editautoGroup/'+response.json().id]);		
			...
			this.http.post(API_URL+'/AutoGroups/update?where=%7B%22id%22%3A%22'+  this.editparam.id +'%22%7D&access_token='+ localStorage.getItem('currentUserToken'), this.data,  options)
	        .subscribe(response => {
	        	console.log(response.json());			
				this.router.navigate(['editautoGroup/'+response.json().id]);
			   // this.router.navigate(['auto-group']);  
