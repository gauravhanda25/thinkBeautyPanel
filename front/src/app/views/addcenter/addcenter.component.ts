...
/* 			
	  ...
	  /*   let roles = NgxRolesService.getRoles();
	    ...
	    if(this.editparam.id != undefined) {
	    	...
	    	this.http.get(API_URL+'/Centers/'+ this.editparam.id +'?access_token='+ localStorage.getItem('currentUserToken'), options)
	        .subscribe(response => {
	        	console.log(response.json());	
	...
	onSubmit() {
		...
		if(this.editparam.id  == undefined){
			...
			this.http.post(API_URL+'/Centers?access_token='+localStorage.getItem('currentUserToken'), this.data,  options)
			.subscribe(response => {
				console.log(response.json());		
			...
			this.http.post(API_URL+'/Centers/update?where=%7B%22id%22%3A%22'+  this.editparam.id +'%22%7D&access_token='+ localStorage.getItem('currentUserToken'), this.data,  options)
	        .subscribe(response => {
	        	console.log(response.json());			
			    this.router.navigate(['center']);  
		    }, error => {
