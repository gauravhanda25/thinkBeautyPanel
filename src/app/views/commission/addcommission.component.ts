...
export class AddcommissionComponent {
    ...
    constructor(private NgxRolesService: NgxRolesService, private NgxPermissionsService: NgxPermissionsService, @Inject(Http) private http: Http, @Inject(Router)private router:Router, private activatedRoute: ActivatedRoute, toasterService: ToasterService) {
	   ...
	   let roles = NgxRolesService.getRoles();
	    ...
	    if(this.editparam.id != undefined) {
	    	...
	    	this.http.get(API_URL+'/Commissions/'+ this.editparam.id +'?access_token='+ localStorage.getItem('currentUserToken'), options)
	        .subscribe(response => {
	...
	onSubmit() {
		...
		if(this.editparam.id  == undefined){
			...
			this.http.post(API_URL+'/Commissions?access_token='+localStorage.getItem('currentUserToken'), this.data,  options)
			.subscribe(response => {
				
				var toast: Toast = {
				type: 'success',
				title: 'Success',
				body: "Commission added successfully.",
			...
			this.http.post(API_URL+'/Commissions/update?where=%7B%22id%22%3A%22'+  this.editparam.id +'%22%7D&access_token='+ localStorage.getItem('currentUserToken'), this.data,  options)
	        .subscribe(response => {
	        
				var toast: Toast = {
				type: 'success',
				title: 'Success',
				body: "Commission updated successfully.",
				onHideCallback: (toast) => this.router.navigate(['commission'])  
			  };
