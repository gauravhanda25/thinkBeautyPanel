...
@Component({
	templateUrl: 'addhair.component.html',
	styleUrls: ['../../../scss/vendors/toastr/toastr.scss'],
	encapsulation: ViewEncapsulation.None
})

@Injectable()
export class AddhairComponent {
	private data: any;
	  ...
	  if(localStorage.getItem('currentUserRoleId') == "1"){
	   ...
	   let roles = NgxRolesService.getRoles();
	    ...
	    if(this.editparam.id != undefined) {
	    	...
	    	this.http.get(API_URL+'/Hairs/'+ this.editparam.id +'?access_token='+ localStorage.getItem('currentUserToken'), options)
	        .subscribe(response => {
	...
	onSubmit() {
		...
		if(this.editparam.id  == undefined){
			this.http.post(API_URL+'/Hairs?access_token='+localStorage.getItem('currentUserToken'), this.data,  options)
			.subscribe(response => {
				
				var toast: Toast = {
				type: 'success',
				title: 'Success',
				body: "Hair Service Record added successfully.",
				onHideCallback: (toast) => this.router.navigate(['hair'])  
			  };
			...
			this.http.post(API_URL+'/Hairs/update?where=%7B%22id%22%3A%22'+  this.editparam.id +'%22%7D&access_token='+ localStorage.getItem('currentUserToken'), this.data,  options)
	        .subscribe(response => {

				var toast: Toast = {
				type: 'success',
				title: 'Success',
				body: "Hair Service Record updated successfully.",
				onHideCallback: (toast) => this.router.navigate(['hair'])  
			  };
